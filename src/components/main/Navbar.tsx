...
const Navbar = () => {
  ...
  return (
    <div className="fixed top-7 left-0 w-full flex justify-center z-50 px-4">
        ...
        {sections.map((item, index) => (
          <motion.a
            ...
            className={`relative text-sm md:text-base font-semibold transition-all px-6 py-3 ${
              activeSection === item
                ? 'bg-cyan-500 text-white font-bold rounded-full'
      ...
      <motion.div initial={{ x: '-100%' }} animate={{ x: isOpen ? 0 : '-100%' }} transition={{ duration: 0.3 }} className="fixed top-0 left-0 w-64 h-full bg-gray-900 text-white z-50 flex flex-col items-start p-6 shadow-lg">
